...
impl VerifyingKey {
    /// Wraps serialized bytes as a Falcon verifying-key carrier.
...
impl SigningKey {
    /// Wraps serialized bytes as a Falcon signing-key carrier.
    pub fn from_bytes(bytes: Vec<u8>) -> Self {
        Self { bytes }
    }
...
impl Signature {
    /// Wraps serialized bytes as a Falcon detached-signature carrier.
    pub fn from_bytes(bytes: Vec<u8>) -> Self {
        Self { bytes }
    }
