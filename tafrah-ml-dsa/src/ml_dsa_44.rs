...
/// Generates an `ML-DSA-44` keypair from a caller-supplied seed.
pub fn keygen_internal(seed: &[u8; 32]) -> (VerifyingKey, SigningKey) {
    crate::keygen::ml_dsa_keygen_internal(seed, &ML_DSA_44)
        .expect("fixed ML-DSA-44 parameter set must be valid")
}
...
/// Signs a message with the deterministic `ML-DSA-44` variant.
pub fn sign_deterministic(sk: &SigningKey, msg: &[u8]) -> Result<Signature, Error> {
    crate::sign::ml_dsa_sign_deterministic(sk, msg, &ML_DSA_44)
}

/// Signs a message with the deterministic `ML-DSA-44` variant and context.
pub fn sign_deterministic_with_context(
    sk: &SigningKey,
    msg: &[u8],
    ctx: &[u8],
) -> Result<Signature, Error> {
    crate::sign::ml_dsa_sign_deterministic_with_context(sk, msg, ctx, &ML_DSA_44)
}

/// Signs an externally supplied `mu` value with `ML-DSA-44`.
pub fn sign_extmu(
    sk: &SigningKey,
    mu: &[u8; 64],
    rng: &mut (impl rand_core::CryptoRng + rand_core::Rng),
...
/// Deterministically signs an externally supplied `mu` value with `ML-DSA-44`.
pub fn sign_extmu_deterministic(sk: &SigningKey, mu: &[u8; 64]) -> Result<Signature, Error> {
    crate::sign::ml_dsa_sign_extmu_deterministic(sk, mu, &ML_DSA_44)
}
...
pub fn sign_prehash_shake256_deterministic(
    ...
    crate::sign::ml_dsa_sign_prehash_shake256_deterministic(sk, msg, ctx, &ML_DSA_44)
}

/// Signs and returns `signature || message` for `ML-DSA-44`.
pub fn sign_message(
    sk: &SigningKey,
    msg: &[u8],
    rng: &mut (impl rand_core::CryptoRng + rand_core::Rng),
...
/// Signs and returns `signature || message` for `ML-DSA-44` with context.
...
/// Verifies an `ML-DSA-44` signature over an externally supplied `mu`.
pub fn verify_extmu(vk: &VerifyingKey, mu: &[u8; 64], sig: &Signature) -> Result<(), Error> {
    crate::verify::ml_dsa_verify_extmu(vk, mu, sig, &ML_DSA_44)
}

/// Verifies a SHAKE256 HashML-DSA signature for `ML-DSA-44`.
pub fn verify_prehash_shake256(
    ...
    crate::verify::ml_dsa_verify_prehash_shake256(vk, msg, sig, ctx, &ML_DSA_44)
}

/// Verifies and opens a signed `ML-DSA-44` message.
pub fn open_signed_message(
    vk: &VerifyingKey,
    signed_message: &SignedMessage,
) -> Result<alloc::vec::Vec<u8>, Error> {
    crate::verify::ml_dsa_open_signed_message(vk, signed_message, &ML_DSA_44)
}

/// Verifies and opens a signed `ML-DSA-44` message with context.
pub fn open_signed_message_with_context(
    vk: &VerifyingKey,
    signed_message: &SignedMessage,
    ctx: &[u8],
) -> Result<alloc::vec::Vec<u8>, Error> {
    crate::verify::ml_dsa_open_signed_message_with_context(vk, signed_message, ctx, &ML_DSA_44)
}
