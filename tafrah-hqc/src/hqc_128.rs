...
/// Marker type for generic `HQC-128` code via [`Kem`].
pub struct Hqc128Kem;

/// Generates an `HQC-128` keypair.
pub fn keygen(
    rng: &mut (impl rand_core::CryptoRng + rand_core::Rng),
) -> Result<(EncapsulationKey, DecapsulationKey), Error> {
    crate::keygen::hqc_keygen(rng, &HQC_128)
}

/// Encapsulates a shared secret for an `HQC-128` public key.
pub fn encapsulate(
    ek: &EncapsulationKey,
    rng: &mut (impl rand_core::CryptoRng + rand_core::Rng),
) -> Result<(Ciphertext, SharedSecret), Error> {
    crate::encaps::hqc_encaps(ek, rng, &HQC_128)
}

/// Decapsulates an `HQC-128` ciphertext.
pub fn decapsulate(dk: &DecapsulationKey, ct: &Ciphertext) -> Result<SharedSecret, Error> {
    crate::decaps::hqc_decaps(dk, ct, &HQC_128)
}

impl Kem for Hqc128Kem {
